...
function savesearch_go(id){
	...
	var savego = jQuery("#saveseachgo"+id).val();
	var myArray = savego.split("]");
	...
	jQuery('.select2-selection__rendered').html('Any');
	jQuery('#select2-reg_field_tax_height_side-container').html('Min. Height');
	jQuery('#select2-reg_field_tax_height_side_max-container').html('Max. Height');
	jQuery('.customfilter').each(function(){
		if(jQuery(this).data('type') == "checkbox"){
			jQuery(this).removeAttr('checked');
		}
		if(jQuery(this).data('type') == "select"){
			console.log('select result');
			console.log(result);
			jQuery('.customfilter option').each(function(){
	...
	console.log('citybystate result');
	console.log(result);
	
	jQuery.each(result, function(index, value){
		if (index.indexOf('min') > -1 || index.indexOf('max')){
			var ind = index.split("-");
			var Indval = ind.pop();
			var indName = ind.join("-");
			ind = indName;
		}else{
			var ind = index.split("-");
			ind = ind[0];
		...
		if (value.indexOf('-') > -1){
			var arr = value.split("-");
			var taxonomiesval = arr.pop();
			var taxonomiesName = arr.join("-");
			
		}else{
			taxonomiesName = value;
		}
		if(taxonomiesName == 'spiritual-path'){
			taxonomiesName = 'spiritual_path';
		}
		
		if(ind == 'citybystate'){
			if(taxonomiesName == null){
				jQuery('#filter-citybystate').nextAll('span:first').css("display", "none"); 
			}
		}
		jQuery('.customfilter').each(function(index,item){
			...
			if(jQuery(this).data('type') == "select"){
				var selectID ='';
				var selectValueOtion ='';
				var selectcontainer ='';
				var select_valoption ='';
				
				if(ind == 'country'){
					jQuery("#filter-city").select2({ theme: 'newselect form-control'}); 
            		ajax_update_citylist();
					selectID = 'filter-'+ind;
					selectValueOtion = jQuery('#'+selectID);
					jQuery('#'+selectID).trigger('change');
					selectcontainer = jQuery('#select2-'+selectID+'-container');
				}else if(ind == 'city'){
					selectID = 'filter-'+ind;
					selectValueOtion = jQuery('#'+selectID);
					selectcontainer = jQuery('#select2-'+selectID+'-container');
				}else if(ind == 'citybystate'){
					selectID = 'filter-'+ind;
					selectValueOtion = jQuery('#'+selectID);
					selectcontainer = jQuery('#select2-'+selectID+'-container');
				}else if(ind == 'taxonomy-min'){
					selectID =  'reg_field_tax_height_side';
					selectValueOtion = jQuery('#'+selectID);
					selectcontainer = jQuery('#select2-reg_field_tax_height_side-container');
				}else if(ind == 'taxonomy-max'){
					selectID =  'reg_field_tax_height_side_max';
					selectValueOtion = jQuery('#'+selectID);
					selectcontainer = jQuery('#select2-reg_field_tax_height_side_max-container');
				}else{
					selectID =  'reg_field_tax_'+taxonomiesName;
					selectValueOtion = jQuery('#reg_field_tax_'+taxonomiesName);
					selectcontainer = jQuery('#select2-reg_field_tax_'+taxonomiesName+'-container');
				}
				
				if(jQuery(this).attr("data-select2-id") == selectID){
					if(ind == 'taxonomy-max'){
						jQuery("#reg_field_tax_height_side_max option").each(function() {
							if(jQuery(this).attr('data-height-cms') == taxonomiesName){
								jQuery(this).attr("selected",true);
							}
						});
						var conceptName = selectValueOtion.find(":selected").text();
					}else if(ind == 'taxonomy-min'){
						jQuery("#reg_field_tax_height_side option").each(function() {
							if(jQuery(this).attr('data-height-cms') == taxonomiesName){
								jQuery(this).attr("selected",true);
							}
						});
						...
						var cityInt = setInterval(function(){
							if(jQuery('#select2-filter-city-container').length > 0){
								clearInterval(cityInt);
								setTimeout(function(){
									console.log('citybystate value');
									console.log(value);
									if(value != null){
										jQuery("#filter-citybystate").select2({theme: 'newselect form-control'});
										ajax_update_citylistbystate();
										selectValueOtion.find('option[value="'+value+'"]').attr("selected",true);
										var conceptName = selectValueOtion.find(":selected").text();
										selectcontainer.html(conceptName);
										selectValueOtion.trigger('change');
										setTimeout(function(){
											selectValueOtion.find('option[value="'+value+'"]').attr("selected",true);
											var conceptName = selectValueOtion.find(":selected").text();
											selectcontainer.html(conceptName);
						...
						var cityStInt = setInterval(function(){
							if(jQuery('#select2-filter-city-container').length > 0){
								clearInterval(cityStInt);
								setTimeout(function(){
									selectValueOtion.find('option[value="'+value+'"]').attr("selected",true);
									var conceptName = selectValueOtion.find(":selected").text();
									selectcontainer.html(conceptName);
			...
			if(jQuery(this).data('type') == "text"){
				if(jQuery(this).attr("data-key") == ind){
					jQuery(this).val(value);
				}
			}
			/* if(jQuery(this).data('key') == "citybystate"){
				var stateInt = setInterval(function(){
					if(jQuery('#select2-filter-citybystate-container').length > 0){
						clearInterval(stateInt);
						setTimeout(function(){
							if(ind == 'citybystate'){
								jQuery('#filter-citybystate option').each(function(){
									if(jQuery(this).val() == taxonomiesName){
										jQuery(this).attr("selected",true);
										var cityBystate = jQuery('#filter-citybystate').find(":selected").text();
										jQuery('#select2-filter-citybystate-container').html(cityBystate);
									}
								});
							}
						},1000);
...
function _filter_update(ele){
    ...
    if(jQuery(ele).data('type') == "checkbox"){
		if (ww > 575.98) {
			var fc = jQuery(ele).closest('.filter-content');
			var fid = jQuery(fc).attr('id');
			jQuery('#'+fid+' input[type=checkbox]').not(ele).prop('checked', false);
			console.log(fid);  
		}else{
			var fc = jQuery(ele).closest('.filter-content_1');
			var fid = jQuery(fc).attr('id');
			jQuery('#'+fid+' input[type=checkbox]').not(ele).prop('checked', false);
			console.log(fid);  
		}
	...
	jQuery('.customfilter').each(function(index,item){
		...
		if(jQuery(this).data('type') == "select" && jQuery(this).val() != "" ){
            ...
            if(jQuery(this).attr('id') == 'reg_field_tax_height_side_max'){
                var cms = jQuery('#reg_field_tax_height_side_max option:selected').data('height-cms');
                value = cms;
                //console.log(cms);        
            }
            if(jQuery(this).attr('id') == 'reg_field_tax_height_side'){
                var cms2 = jQuery('#reg_field_tax_height_side option:selected').data('height-cms');
                value = cms2;
                //console.log(cms);        
            } 
...
function _filter_page(pagenum){	
	var ww = jQuery(window).width();
	 if (ww > 575.98) {
		 ...
		 jQuery("html, body").animate( {scrollTop:jQuery(".filters_listing").offset().top - 70 }, "fast");
	 }
...
function _filter_newsearch(){
    ...
    if(jQuery('.filter_sortby a.active').data('key')=='recommended'){
        jQuery('#rfy').val('1');    
    }else{
        jQuery('#rfy').val('');    
    }
	
	if(jQuery('.filter_sortby a.active').data('key') == 'online'){
        jQuery('#online_S').val('1');    
    }else{
        jQuery('#online_S').val('');    
    }
	
	if(jQuery('.filter_sortby a.active').data('key') == 'wholikeme'){
        jQuery('#WhoLikesMe').val('1');    
    }else{
        jQuery('#WhoLikesMe').val('');    
    }
	if(jQuery('.filter_sortby a.active').data('key') == 'favorites_1'){
        jQuery('#favorites_S').val('1');    
    }else{
        jQuery('#favorites_S').val('');    
    }
	if(jQuery('.filter_sortby a.active').data('key') == 'ChatRequest'){
        jQuery('#chat_request').val('1');
        jQuery('.AADECBUTTON').addClass('SHOWBTN');
        jQuery('.VIEWPROFILEBUTTON').addClass('HIDEBTN');
		
    }else{
        jQuery('#chat_request').val(''); 
		jQuery('.AADECBUTTON').addClass('HIDEBTN');
        jQuery('.VIEWPROFILEBUTTON').addClass('SHOWBTN');		
    }
	...
	jQuery.ajax({
		...
		data: {
			...
			online_S: jQuery('#online_S').val(),
			WhoLikesMe: jQuery('#WhoLikesMe').val(),
			favorites_S: jQuery('#favorites_S').val(),
			chat_request: jQuery('#chat_request').val(),
            rfy: jQuery('#rfy').val()
        },
        success: function(response) { 
			
			//console.log('response221',response);
