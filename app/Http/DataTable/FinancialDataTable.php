...
class FinancialDataTable  {
    ...
    public function __construct(FranchiseFinancial $FranchiseFinancial) {
        $this->FranchiseFinancial = $FranchiseFinancial;
    }

    public function indexList($request){
        $queue = FranchiseFinancial::query();
        ...
        return DataTables::of($queue)   ->addIndexColumn()
            ...
            ->filter(function ($query) use ($request) {
                if ($request->has('search') && !empty($request->search)) {
                    ...
                    $query->orWhere('contact_number', 'like', "%{$request->get('search')['value']}%");
                    $query->orWhere('email', 'like', "%{$request->get('search')['value']}%");
                    $query->orWhere('qualification', 'like', "%{$request->get('search')['value']}%");
                    $query->orWhere('email', 'like', "%{$request->get('search')['value']}%");
                }
            })

            ->editColumn('qualification', function ($queue) {
                return Str::limit($queue->qualification, 50, '....');
				// $status= $queue->status;
            ...
            ->addColumn('action', function ($queue) {
                ...
                <a onclick="return confirm(\'Are you sure to delete request?\')" href="' . route('admin.financial.delete', $parameter) . '" data-toggle="tooltip" data-placement="top" title="" data-original-title="Close"><i class="fa fa-close color-danger"></i></a>
                ';
            })
