<div id="PoptochatayeshaID" class="style__wrapper___1S4gB style__vertical-aligner___3W6ut Poptochatayesha"><div class=""><div class="style__horizontal-aligner___1Qkkr Poptochatayeshainner"><div><div class="style__main___1l57B"><div class="style__content___67str"><div class="style__query-form___2bHZg"><span class="close-icon" id="closePoptochatayesha">&times;</span><a href="https://wa.me/9962831281?text=Hello%20wigomania!" target="_blank"><img class="ayeshapopupimage" src="{{asset('image/ayeshapopupimage.jpg')}}" alt="closePopImage" loading="lazy"></a></div></div></div></div></div></div></div>
<style>
.ayeshapopupimage{
	max-height:30vh;
}
...
.style__vertical-aligner___3W6ut {
    display: none;
    flex-direction: column;
    justify-content: center;
}
.Poptochatayesha.active {
	display: flex;
}
.style__wrapper___1S4gB {
    font-size: 14px;
...
.style__horizontal-aligner___1Qkkr {
    display: flex;
    flex-direction: row;
    justify-content: center;
    max-height: 650px;
...
.style__title___1_x_m {
    font-size: 1.4rem;
    font-weight: 600;
}
.style__subtext___1YjCo {
    color: gray;
}
.style__content___34CoZ {
    display: flex;
    flex-direction: column;
    flex: 1 1 200px;
}
	...
	<optgroup label="Other countries">
		...
		<option data-dial_code="ZW" value="263">Zimbabwe (+263)</option>
                            ...
                            <div class="formfield coupon_otp_box" style="display: none;margin-top:5px;">
                                ...
                                <input type="hidden" name="reachus_id" class="reachus_ids" id="reachus_ids" value="" autocomplete="off">
								 <input type="hidden" name="receivedfrom_add" class="reachus_ids ritu" id="receivedfrom_addID" value="Hair_Wigs_for_women_in_Chennai" autocomplete="off">
...
<script defer async>
 function loadVideo(container) {
        ...
        iframe.src = 'https://www.youtube.com/embed/054TNeu4Log?autoplay=1&mute=1&loop=1&playlist=054TNeu4Log';iframe.width = '350';  // Width in pixels
		iframe.height = '382'; // Height in pixels
	...
	function loadVideodesktop(container) {
        var iframe = document.createElement('iframe');
        iframe.src = 'https://www.youtube.com/embed/054TNeu4Log?autoplay=1&mute=1&loop=1&playlist=054TNeu4Log';iframe.width = '400';  // Width in pixels
		iframe.height = '380'; // Height in pixels
		iframe.frameBorder = '0'; // Optional: Remove the border
        iframe.allow = 'autoplay; encrypted-media';
        iframe.allowFullscreen = true;
        container.innerHTML = '';
...
jQuery(document).ready(function() {
	...
	const popup = document.getElementById('PoptochatayeshaID');
	const pageKey = 'popup_shown_'+currentUrl;
	//alert(currentUrl);
	//alert(pageKey);
	//alert(localStorage.getItem(pageKey));
	if (!localStorage.getItem(pageKey)) {
        // Detect when the user leaves and comes back
        document.addEventListener('visibilitychange', function() {
            if (document.visibilityState === 'visible') {
                // When the user returns to the page, show the popup
	...
	$('.close-icon').click(function() {
        jQuery('#PoptochatayeshaID').removeClass('active');
    })
            ...
            success: function(response) {
                if (response.error) {
                    ...
                    if (response.is_relode == 'yes') {
                        //window.location.assign(thankuUrl); 
							var formData = {
								Name: $('#full_namesformSubmits').val(),
								custemail: $('#email_idformSubmits').val(),
								purpose_of_enquery: $('#purpose_of_enqueryformSubmits').val(),
								number: $('#phoneformSubmits').val(),
								DialCode:$('#dial_codeformSubmits').val(),
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.error) {
                    ...
                    if (response.is_relode == 'yes') {
							var formData = {
								Name: $('#full_nameFormsubmit').val(),
								custemail: $('#emailFormsubmit').val(),
								purpose_of_enquery: $('#purpose_of_enqueryFormsubmit').val(),
								number: $('#phonesFormsubmit').val(),
								DialCode:'91',
								DiscountCode:response.coupon,
								form:'Google Search'
							};
